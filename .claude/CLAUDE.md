...
```
用户输入 → [Workflow Matcher] → [Planner] → [Executor] → [Reflector] → 结构化报告
                   ↑                                │              │
                   └── 预设场景模板 ──────────────────┘              │
                                      └── 未完成 → 重新分派 ───────┘
```

- **Workflow** (`workflow/`) — 场景层，预定义任务类型模板（tech_research / comparison / default），指导 Planner 拆解策略和 Reflector 校验标准
- **Planner** (`core/planner.py`) — LLM 将任务拆解为 2-6 个步骤，为每步分配工具
- **Executor** (`core/executor.py`) — 顺序执行步骤：工具调用（搜索/文件）或 LLM 推理。反编造：搜索无结果时必须说"未找到"，不能编造。引用绑定：每条事实声明必须带 `(source: URL)`
- **Reflector** (`core/reflector.py`) — 执行后检查任务完成度，返回 JSON。未完成时缺失步骤回 Planner 重新分配工具。校验引用完整性
...
- **Evidence Store** — 结构化证据系统，`self.state["evidence"]` 存储带 ID/事实/来源URL 的证据条目，跨步骤持久化

### 工具系统

`BaseTool` 抽象类在 `core/tool_system.py`。工具注册到 `ToolRegistry` 中，当前有：
- `web_search` (`tools/web_tools.py`) — DuckDuckGo 搜索（免费，无需 API key），含中文→英文翻译层
- `read_webpage` (`tools/web_reader.py`) — 网页正文纯净文本提取（基于 html.parser）
...
# 直接传参运行（自动匹配 workflow）
python main.py "对比 FastAPI 和 Flask"    # → comparison workflow
python main.py "调研 2025 AI Agent 框架"   # → tech_research workflow
python main.py "写一首诗"                 # → default workflow
